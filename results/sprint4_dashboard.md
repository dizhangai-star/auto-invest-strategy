...
`docs/dashboard.html` for GitHub Pages) renders its Plotly views as sidebar tabs — a left
panel of links, one view shown at a time — from the committed
...
- **Outcome distribution** (lead view, default tab): XIRR + final-multiple histograms,
  QQQ vs SPY, both scenarios, with the p10/p50/p90 and win-rate stats inline.
- **Percentile fan**: p10–p90 band + p50 line of wealth-multiple vs elapsed years,
  from `fan_*.csv` — the spread stays front-and-center, never one line.
- **Bull/bear cycles**: growth-of-$1 (log) with QQQ bear spans (≤ −20%, peak→recovery)
...
- **One path (illustrative)**: weekly DCA buys on the full-history QQQ path — **labelled as
